...
bool COVMAT_present = false;

bool correct_gamma = true;

bool use_own_PID_electron = false; //false;   
bool use_own_PID_proton =   false;//false;
bool use_own_PID_neutron =  false;
bool use_own_PID_pip =      true;//false;
bool use_own_PID_pim =      true;//false;
bool use_own_PID_Kp =       true;//false;
bool use_own_PID_Km =       true;//false;
bool use_own_PID_photon =   true;//false;
...
const static int TRAJ_BUFFER = 150;
...
/// List of CLAS12 hipo branches
   ...
   // REC::CovMat 
   vector<int> *vCovMat_pindex = 0;
   vector<float> *vCovMat_c11 = 0;
   vector<float> *vCovMat_c12 = 0;
   vector<float> *vCovMat_c13 = 0;
   vector<float> *vCovMat_c14 = 0;
   vector<float> *vCovMat_c15 = 0;
   vector<float> *vCovMat_c21 = 0;
   vector<float> *vCovMat_c22 = 0;
   vector<float> *vCovMat_c23 = 0;
   vector<float> *vCovMat_c24 = 0;
   vector<float> *vCovMat_c25 = 0;
   vector<float> *vCovMat_c33 = 0;
   vector<float> *vCovMat_c34 = 0;
   vector<float> *vCovMat_c35 = 0;
   vector<float> *vCovMat_c44 = 0;
   vector<float> *vCovMat_c45 = 0;
   vector<float> *vCovMat_c55 = 0;
...
/// general particle properties and additional particle bank properties for non identified particles:
  ...
  float part_CovMat_c11[BUFFER], part_CovMat_c12[BUFFER], part_CovMat_c13[BUFFER], part_CovMat_c14[BUFFER], part_CovMat_c15[BUFFER];
  float part_CovMat_c22[BUFFER], part_CovMat_c23[BUFFER], part_CovMat_c24[BUFFER], part_CovMat_c25[BUFFER];
  float part_CovMat_c33[BUFFER], part_CovMat_c34[BUFFER], part_CovMat_c35[BUFFER];
  float part_CovMat_c44[BUFFER], part_CovMat_c45[BUFFER];
  float part_CovMat_c55[BUFFER];
...
/// partcile variables for identified particles:
  ...
  // added support for the Covariance Matrix
  TMatrixD covmat_ele[BUFFER];
  TMatrixD covmat_prot[BUFFER];
  TMatrixD covmat_Kp[BUFFER];
  TMatrixD covmat_Km[BUFFER];
  //TMatrixD covmat_ele[BUFFER];
...
// --> vectors are used to optimize the file size
  ...
  vector<int> Km_det;
  vector<int> phot_det;
  vector<TMatrixD> ele_covmat;
  vector<TMatrixD> prot_covmat;
  vector<TMatrixD> Kp_covmat;
  vector<TMatrixD> Km_covmat;
...
/// /////////////////////////////////////////////////////////////////////////////
    ...
    out_tree.Branch("ele_covmat",&ele_covmat);
    out_tree.Branch("prot_covmat",&prot_covmat);
    out_tree.Branch("Kp_covmat",&Kp_covmat);
    out_tree.Branch("Km_covmat",&Km_covmat);
...
if(MC_Particle){
 ...
 if( COVMAT_present ){
   anaTree->SetBranchAddress("REC_CovMat_pindex",&vCovMat_pindex);
   anaTree->SetBranchAddress("REC_CovMat_c11",&vCovMat_c11);
   anaTree->SetBranchAddress("REC_CovMat_c12",&vCovMat_c12);
   anaTree->SetBranchAddress("REC_CovMat_c13",&vCovMat_c13);
   anaTree->SetBranchAddress("REC_CovMat_c14",&vCovMat_c14);
   anaTree->SetBranchAddress("REC_CovMat_c15",&vCovMat_c15);
   anaTree->SetBranchAddress("REC_CovMat_c22",&vCovMat_c22);
   anaTree->SetBranchAddress("REC_CovMat_c23",&vCovMat_c23);
   anaTree->SetBranchAddress("REC_CovMat_c24",&vCovMat_c24);
   anaTree->SetBranchAddress("REC_CovMat_c25",&vCovMat_c25);
   anaTree->SetBranchAddress("REC_CovMat_c33",&vCovMat_c33);
   anaTree->SetBranchAddress("REC_CovMat_c34",&vCovMat_c34);
   anaTree->SetBranchAddress("REC_CovMat_c35",&vCovMat_c35);
   anaTree->SetBranchAddress("REC_CovMat_c44",&vCovMat_c44);
   anaTree->SetBranchAddress("REC_CovMat_c45",&vCovMat_c45);
   anaTree->SetBranchAddress("REC_CovMat_c55",&vCovMat_c55);
 }
/// ///////////////////////////////////////////////////////////////
...
ele_covmat.clear(); prot_covmat.clear(); Kp_covmat.clear(); Km_covmat.clear();
...
for(Int_t i = 0; i < BUFFER; i++){ 
  ...
  TMatrixD temp_matrix(5,5);
  //covmat_ele[i] = new TMatrixD(5,5);
...
for(Int_t i = 0; i < BUFFER; i++){ 
  ...
  //std::cout <<" setting crosses to zero " << std::endl;
...
// all particles with negative charge
  ...
  if(FD_eid_charge_check[i]){   // only particles which passed the default pid cut
    ...
    // DC
    //std::cout << " part_DC_c1x[i]  " << part_DC_c1x[i] << std::endl;
...
void assign_particles(void){
  ...
  int MC_Nentries = 0; int CovMat_Nentries = 0;
  ...
  //CovMat_Nentries = vCovMat_pindex->size();
  ...
  if(Traj_Nentries > 0){
    //std::cout << " number of trajectory entries " << Traj_Nentries << std::endl;
    for(int i = 0; i < Traj_Nentries; i++){
      //std::cout << " here entry " << i <<  std::endl;
      //std::cout << " pindex " << vTraj_pindex->at(i) << std::endl;
      //std::cout <<" detid "  << vTraj_detID->at(i) << std::endl;
      if(vTraj_pindex->at(i) >= 0 && vTraj_pindex->at(i) < BUFFER && vTraj_detID->at(i) == 6){    	
	  ...
	  // std::cout << " cx " <<  vTraj_cx->at(i) << " cy " <<  vTraj_cy->at(i) << " cz " <<  vTraj_cz->at(i) << std::endl;
	  //std::cout << " array values " << part_DC_c1z[vTraj_pindex->at(i)] << std::endl;
      }
      if(vTraj_pindex->at(i) >= 0 && vTraj_pindex->at(i) < BUFFER && vTraj_detID->at(i) == 24){  
          part_DC_c2x[vTraj_pindex->at(i)] = vTraj_cx->at(i);
          part_DC_c2y[vTraj_pindex->at(i)] = vTraj_cy->at(i);
          part_DC_c2z[vTraj_pindex->at(i)] = vTraj_cz->at(i);
	  // std::cout << " cx " <<  vTraj_cx->at(i) << " cy " <<  vTraj_cy->at(i) << " cz " <<  vTraj_cz->at(i) << std::endl;
      }
      if(vTraj_pindex->at(i) >= 0 && vTraj_pindex->at(i) < BUFFER && vTraj_detID->at(i) == 36){  
          part_DC_c3x[vTraj_pindex->at(i)] = vTraj_cx->at(i);
          part_DC_c3y[vTraj_pindex->at(i)] = vTraj_cy->at(i);
          part_DC_c3z[vTraj_pindex->at(i)] = vTraj_cz->at(i);
	  //std::cout << " cx " <<  vTraj_cx->at(i) << " cy " <<  vTraj_cy->at(i) << " cz " <<  vTraj_cz->at(i) << std::endl;
      }
    }
  }
  ...
  if( CovMat_Nentries > 0 ){
    for( int i = 0; i < CovMat_Nentries; i++ ){
      std::cout << vCovMat_pindex->at(i) << std::endl;
      if( vCovMat_pindex->at(i) < BUFFER && vCovMat_pindex->at(i) >= 0 ){
	std::cout << " vCovMat_c11 " <<  vCovMat_c11->at(i) << std::endl;
	part_CovMat_c11[vCovMat_pindex->at(i)] = vCovMat_c11->at(i);
	part_CovMat_c12[vCovMat_pindex->at(i)] = vCovMat_c12->at(i);
	part_CovMat_c13[vCovMat_pindex->at(i)] = vCovMat_c13->at(i);
	part_CovMat_c14[vCovMat_pindex->at(i)] = vCovMat_c14->at(i);
	part_CovMat_c15[vCovMat_pindex->at(i)] = vCovMat_c15->at(i);
	part_CovMat_c22[vCovMat_pindex->at(i)] = vCovMat_c22->at(i);
	part_CovMat_c23[vCovMat_pindex->at(i)] = vCovMat_c23->at(i);
	part_CovMat_c24[vCovMat_pindex->at(i)] = vCovMat_c24->at(i);
	part_CovMat_c25[vCovMat_pindex->at(i)] = vCovMat_c25->at(i);
	part_CovMat_c33[vCovMat_pindex->at(i)] = vCovMat_c33->at(i);
	part_CovMat_c34[vCovMat_pindex->at(i)] = vCovMat_c34->at(i);
	part_CovMat_c35[vCovMat_pindex->at(i)] = vCovMat_c35->at(i);
	part_CovMat_c44[vCovMat_pindex->at(i)] = vCovMat_c44->at(i);
	part_CovMat_c45[vCovMat_pindex->at(i)] = vCovMat_c45->at(i);
	part_CovMat_c55[vCovMat_pindex->at(i)] = vCovMat_c55->at(i);
      }
    }
  }
...
void select_electron(int run){
  ...
  for(Int_t i = 0; i < Npart; i++){
    if( i < BUFFER){
      ...
      if(part_status[i] >= 2000 && part_status[i] < 4000){
        ...
        if(FD_eid_default_PID_check[i] ){// && FD_eid_charge_check[i] && FD_eid_EC_outer_vs_EC_inner_check[i]  && FD_eid_EC_hit_position_fiducial_check[i]
	   //&& FD_eid_DC_hit_position_region1_fiducial_check[i] && FD_eid_DC_hit_position_region2_fiducial_check[i] && FD_eid_DC_hit_position_region3_fiducial_check[i] && FD_eid_EC_sampling_fraction_check[i]
      ...
      for(int k = 0; k < Npart; k++){
        if(k < BUFFER){
	      ...
	      mom = sqrt(vpart_px->at(k)*vpart_px->at(k) + vpart_py->at(k)*vpart_py->at(k) + vpart_pz->at(k)*vpart_pz->at(k)); 
              e_ind[i] = k;
              e_count += 1;
	      //std::cout << " electron id " << e_ind[i] << " momentum " << mom << std::endl;
	    }
            check = 0;
	  }
        }
  ...
  for(int i = 0; i < BUFFER; i++){
    if(e_ind[i] != -1){
      ...
      if( COVMAT_present){
	//std::cout << " << eindex " << e_ind[i] << " " << part_CovMat_c11[e_ind[i]] << std::endl;
	TMatrixD temp_covmat(5,5);
	temp_covmat(0,0)=part_CovMat_c11[e_ind[i]];
	temp_covmat(0,1)=part_CovMat_c12[e_ind[i]];
	temp_covmat(0,2)=part_CovMat_c13[e_ind[i]];
	temp_covmat(0,3)=part_CovMat_c14[e_ind[i]];
	temp_covmat(0,4)=part_CovMat_c15[e_ind[i]];
	temp_covmat(1,0)=part_CovMat_c11[e_ind[i]];
	temp_covmat(1,1)=part_CovMat_c22[e_ind[i]];
	temp_covmat(1,2)=part_CovMat_c23[e_ind[i]];
	temp_covmat(1,3)=part_CovMat_c24[e_ind[i]];
	temp_covmat(1,4)=part_CovMat_c25[e_ind[i]];
	temp_covmat(2,0)=part_CovMat_c13[e_ind[i]];
	temp_covmat(2,1)=part_CovMat_c23[e_ind[i]];
	temp_covmat(2,2)=part_CovMat_c33[e_ind[i]];
	temp_covmat(2,3)=part_CovMat_c34[e_ind[i]];
	temp_covmat(2,4)=part_CovMat_c35[e_ind[i]];
	temp_covmat(3,0)=part_CovMat_c14[e_ind[i]];
	temp_covmat(3,1)=part_CovMat_c24[e_ind[i]];
	temp_covmat(3,2)=part_CovMat_c34[e_ind[i]];
	temp_covmat(3,3)=part_CovMat_c44[e_ind[i]];
	temp_covmat(3,4)=part_CovMat_c45[e_ind[i]];
	temp_covmat(4,0)=part_CovMat_c15[e_ind[i]];
	temp_covmat(4,1)=part_CovMat_c25[e_ind[i]];
	temp_covmat(4,2)=part_CovMat_c35[e_ind[i]];
	temp_covmat(4,3)=part_CovMat_c45[e_ind[i]];
	temp_covmat(4,4)=part_CovMat_c55[e_ind[i]];
	temp_covmat.Print();     
	//covmat_ele[i] = &temp_covmat;
      }
...
void select_proton(int run){
    ...
    if( i < BUFFER){
      ...
      for(int k = 0; k < Npart; k++){
	    ...
	    mom = sqrt(vpart_px->at(k)*vpart_px->at(k) + vpart_py->at(k)*vpart_py->at(k) + vpart_pz->at(k)*vpart_pz->at(k)); 
            p_ind[i] = k;
            p_count += 1; 
	    //std::cout << " proton id " << p_ind[i] << " momentum " << mom << std::endl;
	  }
          check = 0;
}
	}  
  ...
  for(int i = 0; i < BUFFER; i++){
    if(p_ind[i] != -1){
      ...
      if( COVMAT_present){
	std::cout << " << prot index " << p_ind[i] << " " << part_CovMat_c11[p_ind[i]] << std::endl;
	TMatrixD temp_covmat(5,5);
	temp_covmat(0,0)=part_CovMat_c11[p_ind[i]];
	temp_covmat(0,1)=part_CovMat_c12[p_ind[i]];
	temp_covmat(0,2)=part_CovMat_c13[p_ind[i]];
	temp_covmat(0,3)=part_CovMat_c14[p_ind[i]];
	temp_covmat(0,4)=part_CovMat_c15[p_ind[i]];
	temp_covmat(1,0)=part_CovMat_c12[p_ind[i]];
	temp_covmat(1,1)=part_CovMat_c22[p_ind[i]];
	temp_covmat(1,2)=part_CovMat_c23[p_ind[i]];
	temp_covmat(1,3)=part_CovMat_c24[p_ind[i]];
	temp_covmat(1,4)=part_CovMat_c25[p_ind[i]];
	temp_covmat(2,0)=part_CovMat_c13[p_ind[i]];
	temp_covmat(2,1)=part_CovMat_c23[p_ind[i]];
	temp_covmat(2,2)=part_CovMat_c33[p_ind[i]];
	temp_covmat(2,3)=part_CovMat_c34[p_ind[i]];
	temp_covmat(2,4)=part_CovMat_c35[p_ind[i]];
	temp_covmat(3,0)=part_CovMat_c14[p_ind[i]];
	temp_covmat(3,1)=part_CovMat_c24[p_ind[i]];
	temp_covmat(3,2)=part_CovMat_c34[p_ind[i]];
	temp_covmat(3,3)=part_CovMat_c44[p_ind[i]];
	temp_covmat(3,4)=part_CovMat_c45[p_ind[i]];
	temp_covmat(4,0)=part_CovMat_c15[p_ind[i]];
	temp_covmat(4,1)=part_CovMat_c25[p_ind[i]];
	temp_covmat(4,2)=part_CovMat_c35[p_ind[i]];
	temp_covmat(4,3)=part_CovMat_c45[p_ind[i]];
	temp_covmat(4,4)=part_CovMat_c55[p_ind[i]];
	std::cout << " proton cov mat " << std::endl;
	temp_covmat.Print();     
	covmat_prot[i] = temp_covmat ;
      }
...
void select_Kplus(int run){
    ...
    if( i < BUFFER){
      ...
      for(int k = 0; k < Npart; k++){
	    ...
	    mom = sqrt(vpart_px->at(k)*vpart_px->at(k) + vpart_py->at(k)*vpart_py->at(k) + vpart_pz->at(k)*vpart_pz->at(k)); 
            Kp_ind[i] = k;
            Kp_count += 1;  
	    //std::cout << " kaon plus id " << Kp_ind[i] << " momentum " << mom << std::endl;
	  }
          check = 0;
}
	}  
  ...
  for(int i = 0; i < BUFFER; i++){
	...
	//std::cout << " << eindex " << e_ind[i] << " " << part_CovMat_c11[e_ind[i]] << std::endl;
	TMatrixD temp_covmat(5,5);
	temp_covmat(0,0)=part_CovMat_c11[Kp_ind[i]];
	temp_covmat(0,1)=part_CovMat_c12[Kp_ind[i]];
	temp_covmat(0,2)=part_CovMat_c13[Kp_ind[i]];
	temp_covmat(0,3)=part_CovMat_c14[Kp_ind[i]];
	temp_covmat(0,4)=part_CovMat_c15[Kp_ind[i]];
	temp_covmat(1,0)=part_CovMat_c11[Kp_ind[i]];
	temp_covmat(1,1)=part_CovMat_c22[Kp_ind[i]];
	temp_covmat(1,2)=part_CovMat_c23[Kp_ind[i]];
	temp_covmat(1,3)=part_CovMat_c24[Kp_ind[i]];
	temp_covmat(1,4)=part_CovMat_c25[Kp_ind[i]];
	temp_covmat(2,0)=part_CovMat_c13[Kp_ind[i]];
	temp_covmat(2,1)=part_CovMat_c23[Kp_ind[i]];
	temp_covmat(2,2)=part_CovMat_c33[Kp_ind[i]];
	temp_covmat(2,3)=part_CovMat_c34[Kp_ind[i]];
	temp_covmat(2,4)=part_CovMat_c35[Kp_ind[i]];
	temp_covmat(3,0)=part_CovMat_c14[Kp_ind[i]];
	temp_covmat(3,1)=part_CovMat_c24[Kp_ind[i]];
	temp_covmat(3,2)=part_CovMat_c34[Kp_ind[i]];
	temp_covmat(3,3)=part_CovMat_c44[Kp_ind[i]];
	temp_covmat(3,4)=part_CovMat_c45[Kp_ind[i]];
	temp_covmat(4,0)=part_CovMat_c15[Kp_ind[i]];
	temp_covmat(4,1)=part_CovMat_c25[Kp_ind[i]];
	temp_covmat(4,2)=part_CovMat_c35[Kp_ind[i]];
	temp_covmat(4,3)=part_CovMat_c45[Kp_ind[i]];
	temp_covmat(4,4)=part_CovMat_c55[Kp_ind[i]];
	temp_covmat.Print();     
	covmat_Kp[i] = temp_covmat;
      }
...
void select_Kminus(int run){
    ...
    if( i < BUFFER){
      ...
      for(int k = 0; k < Npart; k++){
	    ...
	    mom = sqrt(vpart_px->at(k)*vpart_px->at(k) + vpart_py->at(k)*vpart_py->at(k) + vpart_pz->at(k)*vpart_pz->at(k)); 
            Km_ind[i] = k;
            Km_count += 1;  
	    //std::cout << " kaon minus id " << Km_ind[i] << " momentum " << mom << std::endl;
  ...
  for(int i = 0; i < BUFFER; i++){
	...
	//std::cout << " << eindex " << e_ind[i] << " " << part_CovMat_c11[e_ind[i]] << std::endl;
	TMatrixD temp_covmat(5,5);
	temp_covmat(0,0)=part_CovMat_c11[Km_ind[i]];
	temp_covmat(0,1)=part_CovMat_c12[Km_ind[i]];
	temp_covmat(0,2)=part_CovMat_c13[Km_ind[i]];
	temp_covmat(0,3)=part_CovMat_c14[Km_ind[i]];
	temp_covmat(0,4)=part_CovMat_c15[Km_ind[i]];
	temp_covmat(1,0)=part_CovMat_c11[Km_ind[i]];
	temp_covmat(1,1)=part_CovMat_c22[Km_ind[i]];
	temp_covmat(1,2)=part_CovMat_c23[Km_ind[i]];
	temp_covmat(1,3)=part_CovMat_c24[Km_ind[i]];
	temp_covmat(1,4)=part_CovMat_c25[Km_ind[i]];
	temp_covmat(2,0)=part_CovMat_c13[Km_ind[i]];
	temp_covmat(2,1)=part_CovMat_c23[Km_ind[i]];
	temp_covmat(2,2)=part_CovMat_c33[Km_ind[i]];
	temp_covmat(2,3)=part_CovMat_c34[Km_ind[i]];
	temp_covmat(2,4)=part_CovMat_c35[Km_ind[i]];
	temp_covmat(3,0)=part_CovMat_c14[Km_ind[i]];
	temp_covmat(3,1)=part_CovMat_c24[Km_ind[i]];
	temp_covmat(3,2)=part_CovMat_c34[Km_ind[i]];
	temp_covmat(3,3)=part_CovMat_c44[Km_ind[i]];
	temp_covmat(3,4)=part_CovMat_c45[Km_ind[i]];
	temp_covmat(4,0)=part_CovMat_c15[Km_ind[i]];
	temp_covmat(4,1)=part_CovMat_c25[Km_ind[i]];
	temp_covmat(4,2)=part_CovMat_c35[Km_ind[i]];
	temp_covmat(4,3)=part_CovMat_c45[Km_ind[i]];
	temp_covmat(4,4)=part_CovMat_c55[Km_ind[i]];
	temp_covmat.Print();     
	covmat_Km[i] = temp_covmat;
      }
...
bool DC_hit_position_region1_fiducial_cut(int j){
  ...
  double x1_rot = part_DC_c1y[j] * sin((double)sec*60.0*Pival/180.0) + part_DC_c1x[j] * cos((double)sec*60.0*Pival/180.0);
  double y1_rot = part_DC_c1y[j] * cos((double)sec*60.0*Pival/180.0) - part_DC_c1x[j] * sin((double)sec*60.0*Pival/180.0);
  //std::cout << " dcr1 fid " << std::endl;
  //std::cout << " index " << j << std::endl;
  //std::cout << " dc c1y " << part_DC_c1y[j]  << " c1x " << part_DC_c1x[j]  << std::endl;
  //std::cout << " DCR1 CUT: sec " << sec << " x1rot " << x1_rot << " y1rot " << y1_rot << std::endl;
  ...
  if (x1_rot > left && x1_rot > right ) return true; //&& pow(x1_rot,2) > radius2_DCr1) return true;
  else return false;
...
bool DC_hit_position_region1_fiducial_cut_hadrons_positive(int j){
  ...
  //std::cout << " dc sector " << sec << std::endl;
  //std::cout << " dc c1y " << part_DC_c1y[j] << " dc c1x " << part_DC_c1x[j] << std::endl;
  ...
  //std::cout << " x1 rot " << x1_rot << " left " << left << std::endl;
  //std::cout << " x1 rot " << x1_rot << " right " << right << std::endl;
  //std::cout << " x1rot 2 " << pow(x1_rot,2) << " dc radi " << radius2_DCr1 << std::endl;
...
void fill_output_vector_electron(void){
  for(int i = 0; i < BUFFER; i++){
    if(p4_ele[i].E() > 0){
      ...
      ele_covmat.push_back(covmat_ele[i]);
...
void fill_output_vector_proton(void){
  for(int i = 0; i < BUFFER; i++){
    if(p4_prot[i].E() > 0){
      ...
      prot_covmat.push_back(covmat_prot[i]);
...
void fill_output_vector_Kplus(void){
  for(int i = 0; i < BUFFER; i++){
    if(p4_Kp[i].E() > 0){
      ...
      Kp_covmat.push_back(covmat_Kp[i]);
...
void fill_output_vector_Kminus(void){
  for(int i = 0; i < BUFFER; i++){
    if(p4_Km[i].E() > 0){
      ...
      Km_covmat.push_back(covmat_Km[i]);
